...
function calcular(e) {
    ...
    if(parseInt(altura)>0 && parseInt(edad)>0 && sexo && parseInt(peso)>0) {
        ...
        var respuesta="";
        if(pesoIdeal>peso) {
            respuesta="Peso Ideal: "+pesoIdeal+" kg";
            respuesta+="<br>Te faltan "+(pesoIdeal-peso).toFixed(2)+" kg";
        }else if(pesoIdeal<peso){
            respuesta="Peso Ideal: "+pesoIdeal+" Kg";
            respuesta+="<br>Tienes "+(peso-pesoIdeal).toFixed(2)+" kg demas.";
        }else{
            respuesta="Excelente estas en tu peso ideal!";
        }
